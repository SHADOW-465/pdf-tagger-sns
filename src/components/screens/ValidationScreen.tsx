...
export const ValidationScreen: React.FC<ValidationScreenProps> = ({
  ...
  // Celebrate with confetti when achieving 100% score
  useEffect(() => {
    if (score >= 100) {
      confetti({
        particleCount: 90,
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 font-sans">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-200">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-teal-100 text-teal-800 text-xs font-mono mb-2">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium transition-all flex items-center gap-1.5 border border-slate-200"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          ...
          <div className="md:col-span-5 text-center md:text-left flex flex-col sm:flex-row items-center gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                {isPerfect
                  ? 'Zero blocking accessibility violations detected. Screen readers and assistive devices can effortlessly parse document hierarchy.'
                  : 'Actionable compliance recommendations identified. Apply 1-click Quick Fixes below to reach 100% compliance.'}
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-7 grid grid-cols-3 gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-8 text-left">
            <div className="bg-emerald-50/70 p-4 rounded-2xl border border-emerald-100">
              ...
              <div className="text-[10px] text-emerald-700 mt-1">WCAG Criteria</div>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 mb-6 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif font-bold text-lg text-slate-900">
              Accessibility Audit Findings ({report.issues.length})
            </h3>
          </div>
        ...
        {filteredIssues.length === 0 ? (
          <div className="text-center py-10 bg-emerald-50/40 rounded-2xl border border-emerald-100 p-8">
            ...
            <p className="text-xs text-slate-600 max-w-md mx-auto">
              Your document complies with PDF/UA-1 (ISO 14289-1) and WCAG 2.1 AA specifications. You can export the tagged publication now.
            </p>
          </div>
        ) : (
          <div className="space-y-3.5">
            {filteredIssues.map((issue) => (
              <div
                key={issue.id}
                className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all hover:bg-slate-100/60"
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {issue.fixable && (
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-medium shadow-xs transition-all flex items-center gap-1.5"
                    >
                      <Sparkles className="w-3.5 h-3.5 text-teal-200" />
